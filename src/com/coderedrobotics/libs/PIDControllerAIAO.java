 ...
 */
public class PIDControllerAIAO implements ConnectionListener {

    public static final double kDefaultPeriod = .05;
    private OverrideValue m_P;
    private OverrideValue m_I;
    private OverrideValue m_D;
    private OverrideValue m_setpoint;
    private Subsocket errorStream;
    private Subsocket outputStream;
    private Subsocket pid;
...
//    private double m_setpoint = 0.0;
    ...
    public PIDControllerAIAO(double Kp, double Ki, double Kd,
            PIDSource source, PIDOutput output, double period, String name) {
        ...
        try {
            ...
            m_P = new OverrideValue("root.pid." + name + ".p", Kp);
            m_I = new OverrideValue("root.pid." + name + ".i", Ki);
            m_D = new OverrideValue("root.pid." + name + ".d", Kd);
            m_setpoint = new OverrideValue("root.pid." + name + ".setpoint", 0);

            errorStream = root.createNewRoute("root.pid." + name + ".errorgraph");
            outputStream = root.createNewRoute("root.pid." + name + ".outputgraph");
        } catch (NotMultiplexedException | InvalidRouteException ex) {
            Logger.getLogger(PIDControllerAIAO.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public PIDControllerAIAO(double Kp, double Ki, double Kd,
            ...
            String name) {
        this(Kp, Ki, Kd, source, output, kDefaultPeriod, name);
    }
    ...
    private void calculate() {
        ...
        if (enabled) {
            ...
            synchronized (this) {
                m_error = m_setpoint.getValue() - input;
                if (m_continuous) {
                    if (Math.abs(m_error)
                ...
                if (((m_totalError + m_error) * m_I.getValue() < m_maximumOutput)
                        && ((m_totalError + m_error) * m_I.getValue() > m_minimumOutput)) {
                    m_totalError += m_error;
                }

                m_result = (m_P.getValue() * m_error + m_I.getValue() * m_totalError + m_D.getValue() * (m_error - m_prevError));
                m_prevError = m_error;
                ...
                errorStream.sendData(PrimitiveSerializer.toByteArray(m_error));
                outputStream.sendData(PrimitiveSerializer.toByteArray(m_result));
            }
    ...
    public synchronized void setPID(double p, double i, double d) {
        m_P.setOriginalValue(p);
        m_I.setOriginalValue(i);
        m_D.setOriginalValue(d);
    }
    ...
    public synchronized void setInputRange(double minimumInput, double maximumInput) {
        ...
        setSetpoint(m_setpoint.getValue());
    }
    ...
    public synchronized void setSetpoint(double setpoint) {
        if (m_maximumInput > m_minimumInput) {
            if (setpoint > m_maximumInput) {
                m_setpoint.setOriginalValue(m_maximumInput);
            } else if (setpoint < m_minimumInput) {
                m_setpoint.setOriginalValue(setpoint);
            } else {
                m_setpoint.setOriginalValue(setpoint);
            }
        } else {
    ...
    public void connected() {
        pid.sendData(PrimitiveSerializer.toByteArray(name));
    }
